...
import {SearchByProductName, SERVER_URL} from "../services/Service";
...
export default function ProductList() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const {data: dataSearch} = await SearchByProductName(productName.toString());
                setProduct(dataSearch['productCountResponses'])
                console.log(product.map(item => item))
            } catch (err) {
    ...
    return (
        ...
        <div className="container m-0 m-md-0 m-lg-5 user-select-none">
            <div className="d-flex flex-column flex-md-column flex-lg-row">
                <div className="col-12 col-lg-3 d-none d-md-flex flex-lg-column">
                    <div className="border shadow rounded-3 p-3 mx-2">
                        <div>
                            <span className="fa fa-history" style={{fontSize: '14px'}}/>
                            <span className="fw-bold mx-2">جستجو محصولات</span>
                        </div>
                        ...
                        <div className="d-flex my-3 align-items-center">
                            ...
                            {
                                name === "" ?
                                    <div className="mx-3 link-secondary opacity-25">
                                        <span className="fa fa-search fs-5"/>
                                    </div> :
                                    <a
                                        className="mx-3"
                                        href={'/product/' + name}>
